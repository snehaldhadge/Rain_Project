# This code will download all the required World Bank Data Indicator Files
# The Indicators are mentioned in the World_Bank_data_indicators word document
# It combines all the latest data from the website into WorldBank Combined Data LATEST.xlsx
import sys
import requests
import pandas as pd
...
writer = pd.ExcelWriter('WorldBank Combined Data LATEST.xlsx', engine='xlsxwriter')

# This reads the Excel file downloaded from the website
# We do not need data from 1960-1999 so dropping those columns
def get_DataFrame(url,f_name,sname):
    ...
    df = excel_df.drop(excel_df.iloc[:,2:44],axis=1)
    df.to_excel(writer, sheet_name=sname, index=None)
def main():

    ### Economy and Growth Section
